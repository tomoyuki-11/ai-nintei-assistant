...
出力ルール（厳守）
出力は番号（半角スペースを入れる）項目名（全角スペースを入れる）判定。（全角スペースを入れる）特記：〇〇の形で出力してください（番号 項目名　判定。　特記：〇〇）
出力は箇条書き（1行＝1項目）
判定は太文字で表示してください。
形式は次のどちらかに統一する
...
対象項目（この順に必ず全て出力）
概況： 居住環境・ 家族構成・介護者の有無・関係・サービス利用状況（訪問介護・デイサービスなど）・家族の介護能力・負担状況・本人・家族の希望や懸念点を箇条書きにせず文章にすること。※個人が特定できる内容（氏名、施設名、病院名、地域名）は表示しないこと。（厳守）
